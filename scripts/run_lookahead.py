"""
Run a complete episode of FleetReplacementEnv using the LookaheadAgent policy.

The agent solves a deterministic MILP planning model at every step and returns
the first-period decisions as Gymnasium actions.  Detailed per-step output shows
the year, per-slot decisions, fleet state, market prices, and reward breakdown.

Usage
-----
    python scripts/run_lookahead.py
    python scripts/run_lookahead.py --config configs/env.yaml --seed 42
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run FleetReplacementEnv for one episode using the LookaheadAgent.",
